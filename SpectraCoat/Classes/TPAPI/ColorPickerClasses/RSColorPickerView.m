...
- (void)initRoutine
{
    ...
    _selectionView = [[RSSelectionView alloc] initWithFrame:CGRectMake(0.0, 0.0, 22, 22)];
...
-(void)didMoveToWindow {
    ...
    UIImage * img = [_colorImage imageByScalingToSize:CGSizeMake(self.frame.size.width * (isRetinaDevice() ? 2 : 1) , self.frame.size.height * (isRetinaDevice() ? 2 : 1) )];
    _rep = [[ANImageBitmapRep alloc] initWithImage:img];
...
- (UIColor*)colorAtPoint:(CGPoint)point {
	...
	return [UIColor colorWithHue:h saturation:s brightness:v alpha:_opacity];
}
...
//	_brightnessView.alpha = 1 - _brightness;
	_gradientView.alpha = 1;//_brightness;
